...
if __name__ == '__main__':
    ...
    dict_cur.execute("CREATE TABLE village (id serial PRIMARY KEY, name varchar(255) NOT NULL, password varchar(255), state int REFERENCES state(id) DEFAULT 1, villager_number integer DEFAULT 0, werewolf_number integer DEFAULT 0, fortuneteller_number integer DEFAULT 0, thief_number integer DEFAULT 0, madman_number integer DEFAULT 0, hanging_number integer DEFAULT 0, talking_time integer DEFAULT 0, ending_time timestamp, field_position1 int REFERENCES position(id), field_position2 int REFERENCES position(id), winner_side int REFERENCES position(id))")
    conn.commit()
